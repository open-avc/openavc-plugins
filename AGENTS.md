...
SURFACE_LAYOUT = {
    ...
    "max_pages": 10,             # Static-grid plugins only (see below)
...
**Device-backed surfaces (`requires_device`):** when set, the IDE never renders the static grid as if hardware were attached. With zero units connected it shows a connect prompt instead ("No <device_label> detected... connect by USB"), plus an add-virtual-unit option when `virtual_models` is declared (the model strings must match what the plugin's own virtual-unit config accepts). Plugins that omit `requires_device` keep the static-grid behavior and the `max_pages` page cap. The IDE renders the plugin's `CONFIG_SCHEMA` settings form at the bottom of the surface view when the schema is non-empty -- a surface plugin whose settings are all edited in context (the Stream Deck plugin) can omit `CONFIG_SCHEMA` entirely and no settings form renders anywhere.

**The workbench editor (device-backed surfaces):** for `requires_device` plugins the IDE draws the unit as a live picture (the mirror convention below) with a persistent inspector rail: clicking a control edits it, Shift+click presses it (`simulate_input`), and with nothing selected the rail shows the unit itself (name, status, per-unit settings, Identify). Page tabs are **emergent** -- the editor derives the page count from the config content (entry placements, page names, paging rules, numeric navigate targets) rather than `max_pages`, and the runtime should clamp navigation the same way. Tab switches send the plugin's `set_page` context action and follow `<serial>.current_page`, so the editor and the hardware can never show different pages.
...
**Multiple physical units:** a surface plugin that supports more than one attached device publishes `plugin.<id>.deck_serials` (comma-separated serials, connect order) and per-unit keys at `plugin.<id>.<serial>.*` (`connected`, `model`, and the geometry keys above, plus `current_page`). With one unit the editor stays card-free (the unit's identity lives on the picture and the idle inspector); with several known units it shows selection cards, including dimmed cards for remembered units that are not connected. Per-unit config overrides live in a top-level `decks` map keyed by serial (a unit without an entry mirrors the flat config — presented to users as "the shared layout" vs. "this unit's own layout"; never expose the storage terms). Keep *unit properties* (friendly name, base brightness) in top-level maps **outside** the override (`deck_names`, `deck_settings`) so setting them never forks a layout. Surface entries that must stay identical on every page live in a `global_buttons` array (same shape as `buttons`, no `page` field; an entry there wins over any per-page entry at the same index). See the Stream Deck plugin for the reference implementation.
...
- Accept a `simulate_input` context action (`{serial, type: key|dial_turn|dial_push|touch, index?, pressed?, amount?, x?, y?, touch_type?, x_out?}`) that dispatches into the same handler paths as hardware input — a `key`/`dial_push` without `pressed` is a full press+release tap, and `touch` takes `touch_type: short|long|drag` (drag needs `x_out`) so every gesture is exercisable from the editor and CI — plus a `set_live_mirror` action (`{on: bool}`) so physical units only pay the mirror cost while a Live View is open.
